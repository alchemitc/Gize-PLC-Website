...
export default function ContactPage() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      <section className="bg-gize-navy pt-24 pb-16 lg:pt-32 lg:pb-20 relative overflow-hidden">
          ...
          <div className="grid lg:grid-cols-5 gap-12">
            <div className="lg:col-span-2 space-y-6">
              {contactInfo.map((item, index) => (
                <motion.div key={item.title} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: index * 0.1 }} className="flex items-start gap-4">
                  <div className="flex-shrink-0 w-12 h-12 bg-gize-red/10 rounded-xl flex items-center justify-center"><item.icon className="text-gize-red" size={20} /></div>
                  <div><h4 className="font-bold text-gize-navy text-sm uppercase tracking-wide">{item.title}</h4>{item.details.map((d) => <p key={d} className="text-gray-500 text-sm mt-0.5">{d}</p>)}</div>
                </motion.div>
              ))}
            ...
            <div className="lg:col-span-3">
              <form onSubmit={handleSubmit} className="bg-gize-light rounded-2xl p-6 sm:p-8 lg:p-10">
                <div className="grid sm:grid-cols-2 gap-5 mb-5">
                  <div><label className="block text-gize-navy font-semibold text-sm mb-2">Full Name <span className="text-gize-red">*</span></label><Input name="name" value={formData.name} onChange={handleChange} placeholder="John Doe" required className="bg-white border-gray-200 focus:border-gize-red rounded-lg h-11 text-sm" /></div>
                  <div><label className="block text-gize-navy font-semibold text-sm mb-2">Email <span className="text-gize-red">*</span></label><Input name="email" type="email" value={formData.email} onChange={handleChange} placeholder="john@company.com" required className="bg-white border-gray-200 focus:border-gize-red rounded-lg h-11 text-sm" /></div>
                </div>
                <div className="grid sm:grid-cols-2 gap-5 mb-5">
