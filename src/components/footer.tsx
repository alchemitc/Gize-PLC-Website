...
const socialLinks = [
  ...
  { label: 'X / Twitter', href: '#', icon: <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg> },
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 sm:pt-16 pb-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-10 mb-10 sm:mb-14">
          ...
          <div className="col-span-2 md:col-span-1">
            <Link href="/" className="relative w-[100px] sm:w-[120px] h-9 sm:h-10 flex-shrink-0 mb-4 inline-block">
            ...
            <div className="flex gap-3">
              {socialLinks.map((social) => (
                <a key={social.label} href={social.href} aria-label={social.label}
                  className="w-9 h-9 bg-white/10 hover:bg-gize-red rounded-lg flex items-center justify-center text-white/70 hover:text-white transition-all duration-300">
                  {social.icon}
                </a>
              ))}
            </div>
          ...
          {/* Services */}
          <div>
            <h4 className="text-white font-bold text-xs sm:text-sm uppercase tracking-wider mb-4">Services</h4>
            <ul className="space-y-2.5">
              {footerLinks.services.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-white/50 hover:text-gize-orange text-xs sm:text-sm transition-colors duration-300">{link.label}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          {/* Company */}
          <div>
            <h4 className="text-white font-bold text-xs sm:text-sm uppercase tracking-wider mb-4">Company</h4>
            <ul className="space-y-2.5">
          ...
          {/* Legal */}
          <div>
            <h4 className="text-white font-bold text-xs sm:text-sm uppercase tracking-wider mb-4">Legal</h4>
            <ul className="space-y-2.5">
        ...
        <div className="border-t border-white/10 pt-6 sm:pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-white/30 text-[10px] sm:text-xs">&copy; {new Date().getFullYear()} Gize PLC. All rights reserved.</p>
          <span className="text-white/30 text-[10px] sm:text-xs">#GIZEPLC</span>
        </div>
      </div>
    </footer>
  );
